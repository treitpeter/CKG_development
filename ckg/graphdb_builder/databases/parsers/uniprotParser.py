    ...
    for url in file_urls:
        ...
        with open(fileName, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                data = line.rstrip("\r\n").split("\t")
                peptide = data[0]
                # Column index changed: was 6, now 5 (Accessions column)
                accs_col = 5 if len(data) == 6 else 6
                if accs_col >= len(data):
                    continue
                # Accessions may contain semicolons and commas
                accs_raw = data[accs_col].replace(";", ",")
