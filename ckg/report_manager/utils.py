...
try:
    ...
    pisa = None  # Optional: requires pycairo system libraries
import json
...
def convert_html_to_pdf(source_html, output_filename):
    """Convert HTML to PDF. Falls back to saving as HTML if pisa unavailable."""
    if pisa is None:
        # Fallback: save as HTML instead
        html_filename = output_filename.replace('.pdf', '.html')
        with open(html_filename, 'w') as f:
            f.write(source_html)
        print(f"PDF export unavailable (xhtml2pdf not installed). Saved as HTML: {html_filename}")
